...
void visualize(int x = -1, int y = -1, int z = -1) {
    ...
    for (int i = 0; i <= SCREEN_WIDTH - rectSize; i += rectSize) {
        ...
        SDL_Rect rect = { i, 0, rectSize, arr[j] };
        if (complete) {
            SDL_SetRenderDrawColor(renderer, 0, 255, 0, 0);  // Green for sorted bars
            SDL_RenderDrawRect(renderer, &rect);
        }
        else if (j == x || j == z) {
            SDL_SetRenderDrawColor(renderer, 255, 0, 0, 0);  // Red for the bars being compared/swapped
            SDL_RenderFillRect(renderer, &rect);
        }
        else if (j == y) {
            SDL_SetRenderDrawColor(renderer, 255, 0, 0, 0);  // Red for the pivot or selected bar
            SDL_RenderFillRect(renderer, &rect);
        }
        else {
            SDL_SetRenderDrawColor(renderer, 170, 183, 184, 0);  // Default
            SDL_RenderDrawRect(renderer, &rect);
        }
        j++;
    }
